...
void Library::load_from(const string& filename) {
    ...
    // Read game data in groups of 
    while (getline(file, game.title) && getline(file, game.publisher) &&
           getline(file, game.genre) && getline(file, line)) {

        game.hoursPlayed = stof(line); //Convert line to float
        getline(file, line);
        game.price = stof(line); // Convert line to float
        getline(file, line);
        game.year = stoi(line);  //Convert line to int
